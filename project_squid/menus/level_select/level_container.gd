...
func set_level_time():
	...
	if level_progress_times.has(level_name + "(f)"):
		level_time_flipped = level_progress_times[level_name + "(f)"]


func populate_time_text(time):
	if time != -1:
		$Control/Time.set_text("[center]" + format_time(time) + "[/center]")
	else:
...
func flip():
	...
	if is_flipped:
		...
		is_flipped = true
		populate_name_text(level_name.reverse().to_lower().capitalize())
		populate_star_text(level_star_count_flipped)
		populate_time_text(level_time_flipped)
