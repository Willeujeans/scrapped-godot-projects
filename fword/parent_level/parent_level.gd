...
func _ready():
	...
	find_also_set_next_scene()

func find_also_set_next_scene():
	var levels_array: Array[PackedScene] = AllDiscs.get_discs()[AllDiscs.current_disc].levels
	var current_scene: PackedScene = load(self.get_scene_file_path())
	var current_index = levels_array.find(current_scene)
	var next_index = current_index + 1
	if next_index >= levels_array.size():
		$SceneChangeNode.next_scene = back_scene
	else:
		$SceneChangeNode.next_scene = levels_array[next_index]
...
func _on_area_2d_body_entered(body):
	...
	# we set a new record
	if best_star_count < star_count:
		var current_total_stars = SaveManager.get_data("total_stars")
		current_total_stars += (star_count - best_star_count)
		best_star_count = star_count
		SaveManager.add_to_save("total_stars", current_total_stars)
