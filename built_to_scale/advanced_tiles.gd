...
func update_paint_percent():
	...
	if percentage > 100:
		percentage = 100
	notify_player_of_painted(percentage)
...
func notify_player_of_painted(percentage):
	if $NotificationOfInkedBlock.playing:
		return
	$NotificationOfInkedBlock.pitch_scale = remap(percentage, 0, 100, 0.5, 0.9)
	$NotificationOfInkedBlock.play()
	print("notified")
