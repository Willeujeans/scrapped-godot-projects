...
func explode():
	for each in $ExplosionDetection.get_overlapping_bodies():
		if each.is_class("RigidBody2D"):
			var direction = (each.global_position - Vector2(self.global_position.x, self.global_position.y + 100.0)).normalized()
			#direction.x += Input.get_axis("left","right") * 0.1
...
func _on_ink_detection_area_entered(area):
	if area.is_in_group("ink"):
		...
		if ink_size < body.ink_size:
			...
			body.increase_ink_size(body.get_node('AnimatedSprite2D').scale, body.get_node("HitDetection").get_node('CollisionShape2D').shape.radius, body.get_node("ExplosionDetection").get_node("CollisionShape2D").shape.radius, body.force, modulate)
			queue_free()
