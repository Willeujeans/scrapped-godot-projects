...
func _ready():
	...
	for each in levels:
		...
		if get_tree().get_first_node_in_group("level_progress").level_times.has(str(each.get_state().get_node_name(0))):
			...
			level_container.get_node("Time").set_text("[center]" + formatted_time + "[/center]")
			var stars_from_level = 0
			if saved_time_in_seconds <= int(stars_list[0]):
				stars_from_level += 1
				level_container.get_node("StarCount").set_text("[center]" + "+" + "[/center]")
			...
			if saved_time_in_seconds <= int(stars_list[3]):
				stars_from_level += 1
				level_container.get_node("Button").set("theme_override_styles/normal", load("res://menus/level_select/level_container_button_4Star.tres"))
				level_container.get_node("Button").set("theme_override_styles/focus", load("res://menus/level_select/level_container_button_4Star.tres"))
				level_container.get_node("Button").set_material(load("res://2D_shine_canvas.tres"))
			total_stars += stars_from_level
		else:
			level_container.get_node("Time").set_text("[center]" + "x" + "[/center]")
